...
export class DatabaseStorage implements IStorage {
  ...
  constructor() {
    ...
    this.dataPath = process.env.NODE_ENV === 'production' 
      ...
      : path.join(__dirname, '../data/local-storage.json');
    ...
    try {
      ...
      // Create wrapper store that tries PostgreSQL first, falls back to memory
      this.sessionStore = {
        ...pgStore,
        get: (sid, callback) => {
          if (!this.databaseAvailable) {
            const sess = this.localSessionData.get(sid);
            return callback(null, sess);
          }
          pgStore.get(sid, (err, sess) => {
            if (err) {
              log(`Session get error, using memory fallback: ${err.message}`);
              const localSess = this.localSessionData.get(sid);
              return callback(null, localSess);
            }
            return callback(null, sess);
          });
        },
        set: (sid, sess, callback) => {
          // Always store in local memory for fallback
          this.localSessionData.set(sid, sess);
          
          if (!this.databaseAvailable) {
            return callback?.();
          }
          pgStore.set(sid, sess, (err) => {
            if (err) {
              log(`Session set error, using memory fallback: ${err.message}`);
              this.databaseAvailable = false;
              return callback?.();
            }
            callback?.();
        ...
        destroy: (sid, callback) => {
          this.localSessionData.delete(sid);
          if (!this.databaseAvailable) {
            return callback?.();
          }
          pgStore.destroy(sid, (err) => {
            if (err) {
              log(`Session destroy error: ${err.message}`);
              return callback?.();
            }
            callback?.();
          });
